...
const NavigationBar = () => {
  ...
  // ── NEW: Decode the token when the Navbar loads ──
  useEffect(() => {
    const token = localStorage.getItem('token');
    if (token) {
      try {
        // A JWT has 3 parts separated by dots. The middle part (index 1) is the payload data.
        ...
        // Grab the data Spring Boot saved in the token!
        // Note: Spring usually saves the email as the 'sub' (subject)
          ...
          <div className="d-flex align-items-center gap-4 ps-lg-4 mt-3 mt-lg-0" style={{ borderLeft: '1px solid var(--border)' }}>
            ...
            <div className="position-relative" ref={profileMenuRef}>
              ...
              {showProfileMenu && (
                ...
                >
                  {/* ── UPDATED: Dynamically rendering the real user data! ── */}
                  ...
                  <button 
                    className="btn w-100 text-start mb-2 d-flex justify-content-center align-items-center gap-2 rounded-pill shadow-none" 
                    style={{ color: 'var(--text-main)', border: '1px solid var(--border)', transition: 'background 0.2s', padding: '8px 16px' }}
                    onMouseEnter={(e) => e.currentTarget.style.backgroundColor = 'var(--bg-pill)'}
                    onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}
                    onClick={() => { navigate('/profile'); setShowProfileMenu(false); closeMenu(); }}
                  >
                  ...
                  <button 
                    ...
                    style={{ color: '#dc3545', border: '1px solid rgba(220, 53, 69, 0.3)', transition: 'all 0.2s', padding: '8px 16px' }}
                    onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = 'rgba(220, 53, 69, 0.1)'; e.currentTarget.style.borderColor = '#dc3545'; }}
                    onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.borderColor = 'rgba(220, 53, 69, 0.3)'; }}
